...
const OnboardingPage = () => {
  ...
  // Check for LinkedIn callback data
  useEffect(() => {
    const linkedinToken = searchParams.get('linkedin_token');
    const linkedinDataParam = searchParams.get('linkedin_data');
    ...
    if (linkedinToken && linkedinDataParam) {
      try {
        const data = JSON.parse(decodeURIComponent(linkedinDataParam));
  ...
  const saveLinkedInData = async (data) => {
    try {
      const token = localStorage.getItem('token');
      
      if (!token) {
        console.warn('No auth token found - user may need to log in first');
        return;
      }
      
      const response = await fetch(`${API_URL}/auth/save-linkedin`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        console.log('✅ LinkedIn data saved to user profile');
      } else {
        console.error('Failed to save LinkedIn data:', await response.text());
      }
    } catch (error) {
  ...
  const handleAnswerSubmit = async () => {
    ...
    try {
      ...
      setConversationHistory([
        ...conversationHistory,
        { question: questions[currentQuestionIndex].question, answer: currentAnswer, acknowledgment: followup.acknowledgment }
      ]);
  ...
  const handleLinkedinConnect = () => {
    // Store current progress in localStorage before redirect
  ...
  // Restore progress after LinkedIn redirect
  useEffect(() => {
    const savedProgress = localStorage.getItem('onboardingProgress');
    if (savedProgress && searchParams.get('linkedin_token')) {
      try {
        const progress = JSON.parse(savedProgress);
        setStep(progress.step || 4); // Go to profile completion step
        setSelectedStatus(progress.selectedStatus || '');
        setPassion(progress.passion || '');
        setAnswers(progress.answers || {});
        setConversationHistory(progress.conversationHistory || []);
        setCurrentQuestionIndex(progress.currentQuestionIndex || 0);
        setResumeUploaded(progress.resumeUploaded || false);
        setResumeData(progress.resumeData || null);
  ...
  const handleProfileComplete = async () => {
    ...
    try {
      ...
      console.log('📤 Sending profile analysis request...');
      console.log('Data:', {
      ...
      if (result.success) {
        localStorage.setItem('careerAnalysis', JSON.stringify(result.analysis));
        ...
        <AnimatePresence mode="wait">
                  ...
                  {!answers[questions[currentQuestionIndex]?.id] && (
                    <div>
                      ...
                      <motion.button
                        ...
                        className="w-full mt-4 bg-orange-600 hover:bg-orange-500 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 disabled:opacity-50"
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                      >
                        {aiTyping ? (
          ...
          {step === 4 && (
            <motion.div
              ...
              className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-12"
            >
              <h2 className="text-4xl font-bold text-white mb-4">Complete Your Profile</h2>
              <p className="text-gray-400 mb-8">Upload your resume or connect LinkedIn (both optional, but recommended)</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  ...
                  <label className={`block w-full border-2 border-dashed ${resumeUploaded ? 'border-green-500' : 'border-white/20'} rounded-xl p-8 cursor-pointer hover:border-orange-500 transition-all`}>
                    ...
                    {resumeParsing ? (
                      <div className="text-center">
                        <Loader2 className="w-12 h-12 text-orange-500 mx-auto mb-2 animate-spin" />
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                  ...
                  <button
                    ...
                    className={`w-full ${linkedinConnected ? 'bg-green-500/20 border-green-500' : 'bg-blue-600 hover:bg-blue-500'} border border-transparent text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all mt-4 disabled:cursor-not-allowed`}
                  >
                    {linkedinConnected ? (
                      <>
                  ...
                  {linkedinConnected && linkedinData && (
                    <div className="mt-4 text-center">
                      ...
                      <p className="text-xs text-gray-500">{linkedinData.email}</p>
                    </div>
                  )}
              ...
              {!resumeUploaded && !linkedinConnected && (
                ...
                >
                  <p className="text-blue-400 text-sm flex items-center gap-2">
                    ...
                    While optional, uploading your resume or connecting LinkedIn helps AI provide better career recommendations
                  </p>
                </motion.div>
              )}
