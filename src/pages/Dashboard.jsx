...
const MetricCard = ({ title, value, delta, icon: Icon, colorClass, delay }) => (
  ...
  >
    <div className="flex justify-between items-start mb-4">
      ...
      <span className={`text-xs font-bold flex items-center gap-1 ${delta.includes('+') ? 'text-cyan-400' : 'text-slate-500'}`}>
      ...
      <main className="pt-24 pb-12 px-4 md:px-8 max-w-7xl mx-auto space-y-6">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 font-medium">Your career momentum is up 12% this week.</p>
          </div>
          <div className="flex gap-3">
            <div className="bg-amber-500/10 border border-amber-500/20 px-4 py-2 rounded-2xl flex items-center gap-2">
              <Zap className="w-4 h-4 text-amber-500" />
              <span className="text-sm font-bold text-amber-500">7 Day Streak</span>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard 
            title="Readiness Score" value="84/100" delta="+4%" 
            icon={Target} colorClass="text-amber-500" delay={0.1} 
          />
          <MetricCard 
            title="Skill Level" value="Lvl 14" delta="Next: 240xp" 
            icon={Award} colorClass="text-purple-500" delay={0.2} 
          />
          <MetricCard 
            title="Active Goals" value="6" delta="2 Completed" 
            icon={CheckCircle2} colorClass="text-cyan-400" delay={0.3} 
          />
          <MetricCard 
            title="Thrive Index" value="Premium" delta="Top 5%" 
            icon={Sparkles} colorClass="text-white" delay={0.4} 
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-black text-slate-600 uppercase tracking-widest px-2">Next Actions</h3>
            <div className="bg-purple-500/5 border border-purple-500/20 rounded-3xl p-5 group cursor-pointer hover:bg-purple-500/10 transition-all">
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="text-white font-bold text-sm mb-1">Growth Opportunity</h4>
                  <p className="text-slate-400 text-xs leading-relaxed">Your data visualization skills match a high-demand lead role at Stripe.</p>
            ...
            <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-5 hover:bg-white/[0.06] transition-all">
              <h4 className="text-slate-300 font-bold text-sm mb-3">Strongest Skill This Week</h4>
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-500">System Architecture</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
           <div className="bg-[#080808] border border-white/5 rounded-[2.5rem] p-8">
              ...
              <div className="space-y-6">
                {[
                  { label: 'Completed Node.js Advanced Course', date: '2h ago', icon: <CheckCircle2 className="w-4 h-4 text-cyan-400" /> },
                  { label: 'Earned "Visual Thinking" Badge', date: 'Yesterday', icon: <Award className="w-4 h-4 text-amber-500" /> },
                  { label: 'Updated Portfolio Architecture', date: '3 days ago', icon: <ArrowUpRight className="w-4 h-4 text-slate-400" /> }
                ].map((item, i) => (
